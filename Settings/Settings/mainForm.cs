...
namespace Settings
{
    public partial class mainForm : Form
    {
        ...
        private void lblStartBooster_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(Path.Combine(Application.StartupPath, "EzBooster.exe")))
            {
                var proc = new Process();
                proc.StartInfo.FileName = "EzBooster.exe";
        ...
        private void txtGameItem_TextChanged(object sender, EventArgs e)
        {

        }

        private void lblStartBooster_Click_1(object sender, EventArgs e)
        {
            if (mSettings.Accounts.Count == 0)
            {
                if (Thread.CurrentThread.CurrentCulture.Name.StartsWith("fr"))
